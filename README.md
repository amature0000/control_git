...
- `topic` 폴더: `resource` 폴더의 하위에는 `topic` 폴더만 위치할 수 있습니다. 웹페이지 상의 카테고리로 표시됩니다.
- `name` 폴더: `topic` 폴더의 하위에는 `name` 폴더만 위치할 수 있습니다. 웹페이지 상의 파일명으로 표시됩니다.
- `name` 폴더 하위에는 다운로드될 .zip 파일이 포함되어야 하며, 이외의 다른 파일들은 무시됩니다.
- `#d` suffix: `name` 폴더의 마지막에 `#d`를 추가하면 웹페이지 상에서 해당 파일명이 취소선 처리됩니다.
...
### 2 Push
`runme.bat`을 실행합니다. 수정된 파일은 `runme.bat`을 실행하여 업데이트되는 일자를 기준으로 수정일이 표시됩니다.
